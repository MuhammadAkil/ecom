...
stock=this.products.filter(inStock=>inStock.is_in_inventory===true).length;
outStock=this.products.filter(outStock=>outStock.is_in_inventory===false).length;
...
displayProduct(product: any): boolean {
  const isSearchMatch = this.searchText === '' || product.name.toLowerCase().includes(this.searchText);
  const isFilterMatch = this.selected === 'all' || product.is_in_inventory.toString() === this.selected;
  if (!isSearchMatch) {
    this.notFound=true;
    return false;
  }
